package com.elalamiadam.Controller;
...
public class LoginController {
    ...
    @GetMapping("/")
    public String getInfoUser(Principal user, @AuthenticationPrincipal OidcUser oidcUser) {
        StringBuffer usernameInfo = new StringBuffer();

        if(user instanceof UsernamePasswordAuthenticationToken ){
            usernameInfo.append(getUsernamePasswordLoginInfo(user));
        } else if (user instanceof OAuth2AuthenticationToken){
            usernameInfo.append(getOauth2LoginInfo(user,oidcUser));
        }
        return usernameInfo.toString();
    }
